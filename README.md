# TempMail App Website

Static landing site for the TempMail Android app.

## Includes

- app landing page
- Play Store and releases links
- Privacy Policy page
- Terms of Service page
- contact links for support email and Telegram

## App details

- package: `com.vkrm.tempmail`
- Play Store URL: `https://play.google.com/store/apps/details?id=com.vkrm.tempmail`

## GitHub Pages

This repo is structured to work as a plain GitHub Pages site from the root of the `main` branch.
